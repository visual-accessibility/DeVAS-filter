...
static DEVA_gray_image	*canny_base ( DEVA_float_image *input, double st_dev,
			    double high_threshold, double low_threshold,
			    DEVA_float_image **magnitude_p,
			    DEVA_float_image **orientation_p, int auto_thresh );
static DEVA_float_image	*gradient_magnitude ( DEVA_float_image *image,
			    DEVA_float_image **grad_Y_p,
			    DEVA_float_image **grad_X_p );
static void		auto_thresh_values ( DEVA_float_image *magnitude,
			    double *high_threshold, double *low_threshold );
static DEVA_gray_image	*find_edges ( DEVA_float_image *magnitude,
			    DEVA_float_image *grad_Y,
			    DEVA_float_image *grad_X,
			    double high_threshold, double low_threshold );
static DEVA_float_image	*find_orientation ( DEVA_gray_image *edge_map,
			    DEVA_float_image *grad_Y,
			    DEVA_float_image *grad_X );
static void		hysteresis ( DEVA_gray_image* edges );
static void		clean_up_magnitude ( DEVA_gray_image *edge_map,
			    DEVA_float_image *magnitude );
static void		follow ( DEVA_gray_image *edges, int row, int col);
#ifndef PERCENTILE_ALL
static void		hysteresis_label ( DEVA_gray_image *edge_map,
			    DEVA_float_image *magnitude,
			    double high_threshold, double low_threshold );
...
canny_base ( DEVA_float_image *input, double st_dev, double high_threshold,
    double low_threshold, DEVA_float_image **magnitude_p,
    DEVA_float_image **orientation_p, int auto_thresh )
/*
...
gradient_magnitude ( DEVA_float_image *image, DEVA_float_image **grad_Y_p,
	DEVA_float_image **grad_X_p )
/*
 ...
 */
{
    ...
    DEVA_float_image  *magnitude;	/* gradient magnitude */
    DEVA_float_image  *grad_Y;	/* Y component of gradient */
    DEVA_float_image  *grad_X;	/* X component of gradient */

    n_rows = DEVA_image_n_rows ( image );
    n_cols = DEVA_image_n_cols ( image );

    magnitude = DEVA_float_image_new ( n_rows, n_cols );
    grad_Y = DEVA_float_image_new ( n_rows, n_cols );
    grad_X = DEVA_float_image_new ( n_rows, n_cols );

    /* set values near edge to zero */
    for ( row = 0; row < n_rows; row++ ) {
	DEVA_image_data ( magnitude, row, 0 ) = 0.0;
	DEVA_image_data ( magnitude, row, n_cols - 1 ) = 0.0;

	DEVA_image_data ( grad_Y, row, 0 ) = 0.0;
	DEVA_image_data ( grad_Y, row, n_cols - 1 ) = 0.0;

	DEVA_image_data ( grad_X, row, 0 ) = 0.0;
	DEVA_image_data ( grad_X, row, n_cols - 1 ) = 0.0;
    }
    for ( col = 1; col < n_cols - 1; col++ ) {
	DEVA_image_data ( magnitude, 0, col ) = 0.0;
	DEVA_image_data ( magnitude, n_rows - 1, col ) = 0.0;

	DEVA_image_data ( grad_Y, 0, col ) = 0.0;
	DEVA_image_data ( grad_Y, n_rows - 1, col ) = 0.0;

	DEVA_image_data ( grad_X, 0, col ) = 0.0;
	...
	for ( col = 1; col < n_cols - 1; col++ ) {
	    V = ( DEVA_image_data ( image, row + 1, col ) -
		    DEVA_image_data ( image, row - 1, col ) );
	    H = ( DEVA_image_data ( image, row, col + 1 ) -
		    DEVA_image_data ( image, row, col - 1 ) );

	    D_1 = DEVA_image_data ( image, row + 1, col + 1) -
		DEVA_image_data ( image, row - 1, col - 1);
	    D_2 = DEVA_image_data ( image, row - 1, col + 1) -
		DEVA_image_data ( image, row + 1,col - 1 );

	    X = H + ( 0.5 * ( D_1 + D_2 ) );
	    Y = V + ( 0.5 * ( D_1 - D_2 ) );

	    DEVA_image_data ( magnitude, row, col ) =
		( sqrt ( SQR ( X ) + SQR ( Y ) ) );
	    DEVA_image_data ( grad_Y, row, col ) = Y;
	    DEVA_image_data ( grad_X, row, col) = X;
	}
    }
 ...
 */
{
	...
	for ( col = 1; col < n_cols - 1; col++ ) {
	    ...
	    if ( DEVA_image_data ( magnitude, row, col ) > magnitude_max ) {
		magnitude_max = DEVA_image_data ( magnitude, row, col );
	    }
	}
    }
 ...
 */
{
    ...
    edge_map = DEVA_gray_image_new ( n_rows, n_cols );
    /* set values near edge to zero */
    for ( row = 0; row < n_rows; row++ ) {
	DEVA_image_data ( edge_map, row, 0 ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, row, 1 ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, row, n_cols - 2 ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, row, n_cols - 1 ) = CANNY_NO_EDGE;
    }
    for ( col = 2; col < n_cols - 2; col++ ) {
	DEVA_image_data ( edge_map, 0, col ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, 1, col ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, n_rows - 2, col ) = CANNY_NO_EDGE;
	DEVA_image_data ( edge_map, n_rows - 1, col ) = CANNY_NO_EDGE;
    }
	...
	for ( col = 2; col < n_cols - 2; col++ ) {
	    ...
	    grad_Y_v = DEVA_image_data ( grad_Y, row, col );
	    grad_Y_v_abs = fabs (grad_Y_v );
	    grad_X_v = DEVA_image_data ( grad_X, row, col );
		...
		if ( grad_Y_v > 0.0 ) {
		    ...
		    A_hv_plus = DEVA_image_data ( magnitude, row + 1, col );
		    A_hv_minus = DEVA_image_data ( magnitude, row - 1, col );
		    if ( grad_X_v > 0.0 ) {
			/* gradient points (slighly) right */
			A_d_plus =
			    DEVA_image_data ( magnitude, row + 1, col + 1 );
			A_d_minus =
			    DEVA_image_data ( magnitude, row - 1, col - 1 );
		    } else {
		...
		if ( grad_X_v > 0.0 ) {
		    ...
		    A_hv_plus = DEVA_image_data ( magnitude, row, col + 1 );
		    A_hv_minus = DEVA_image_data ( magnitude, row, col - 1 );
		    if ( grad_Y_v > 0.0 ) {
	     ...
	     * coorinate of DEVA_image_data distance 2 away.  Use this
		...
		if ( ( grad_X_v * grad_Y_v ) > 0.0 ) {
		    ...
		    A_2g_plus = ( ( coordinate - coord_low_f ) *
			    DEVA_image_data ( magnitude, row + 2,
				col + coord_low ) ) +
		...
		if ( ( grad_X_v * grad_Y_v ) > 0.0 ) {
		    ...
		    A_2g_plus = (( coordinate - coord_low_f) *
			    DEVA_image_data ( magnitude, row + coord_low,
				col + 2 ) ) +
			( (coord_high_f - coordinate ) *
			  DEVA_image_data ( magnitude, row + coord_high,
			      col + 2 ) );
 ...
 */
{
    ...
    int		    n_rows, n_cols;
    double	    grad_Y_v, grad_X_v;
    DEVA_float_image  *orientation;

    n_rows = DEVA_image_n_rows ( edge_map );
    n_cols = DEVA_image_n_cols ( edge_map );

    orientation = DEVA_float_image_new ( n_rows, n_cols );

    for ( row = 0; row < n_rows; row++ ) {
	for ( col = 0; col < n_cols; col++ ) {
	    {
		if ( DEVA_image_data ( edge_map, row, col ) == CANNY_NO_EDGE) {
		    DEVA_image_data ( orientation, row, col) =
							CANNY_DIR_NO_EDGE;
		} else {
		    grad_Y_v = DEVA_image_data ( grad_Y, row, col );
		    grad_X_v = DEVA_image_data ( grad_X, row, col );
...
static void
clean_up_magnitude ( DEVA_gray_image *edge_map,  DEVA_float_image *magnitude )
/*
 ...
 */
{
	...
	for ( col = 0; col < n_cols; col++ ) {
	    if ( DEVA_image_data ( edge_map, row, col ) == CANNY_NO_EDGE)
		DEVA_image_data ( magnitude, row, col ) = CANNY_MAG_NO_EDGE;
	}
    }
}

static void
hysteresis ( DEVA_gray_image* edges )
/*
 ...
 */
{
    ...
    n_rows = DEVA_image_n_rows ( edges );
    n_cols = DEVA_image_n_cols ( edges );

    for( row = 0; row < n_rows; row++ ) {
	for ( col = 0; col < n_cols; col++ ) {
	    /* Find a "high" edge. */
	    if ( DEVA_image_data ( edges,row,col) != CANNY_CERTAIN_EDGE)
		continue;

	    DEVA_image_data ( edges,row,col ) = CANNY_MARKED_EDGE;
    ...
    for( row = 0; row < n_rows; row++ ) {
	for ( col = 0; col < n_cols; col++ ) {
	    if ( DEVA_image_data ( edges, row, col ) == CANNY_POSSIBLE_EDGE) {
		DEVA_image_data ( edges, row, col) = CANNY_NO_EDGE;
	    }
	}
    }
}
...
static void
follow ( DEVA_gray_image *edges, int row, int col)
/*
 ...
 */
{
    ...
    for ( row = 2; row < DEVA_image_n_rows ( edge_map ) - 2; row++ ) {
	for ( col = 2; col < DEVA_image_n_cols ( edge_map ) - 2; col++ ) {
	    if ( DEVA_image_data ( magnitude, row, col) <= low_threshold ) {
		/* likely not an * edge */
		DEVA_image_data ( edge_map, row, col ) = CANNY_NO_EDGE;
	    } else if ( DEVA_image_data ( magnitude, row, col ) >
		    high_threshold ) {
		/* pretty sure it's real */
		DEVA_image_data ( edge_map, row, col) = CANNY_CERTAIN_EDGE;
	    } else { /* not really all that certain either way...  */
		DEVA_image_data ( edge_map, row, col) = CANNY_POSSIBLE_EDGE;
	    }
	}
    }
}
