/*
 * Generate displayable version of DEVA geometry file as a PNG file.
 ...
 *
 * 	deva-visualize-geometry --3dx1=y input_xyz.txt output.png
 *
...
char	*Usage =
	    "deva-visualize-geometry [--3dx1=<a>] [--print] [--outline]"
...
main ( int argc, char *argv[] )
{
    ...
    int			geo_dim;
    DEVA_float_image	*data_1d;
    DEVA_XYZ_image	*data_3d;
    DEVA_RGB_image	*display;
    DEVA_RGBf		display_value_float;
    int			row, col;
    ...
    geo_dim = DEVA_geom_dim_from_radfilename ( argv[argpt] );
	...
	data_1d = DEVA_geom1d_from_radfilename ( argv[argpt++] );
	...
	if ( print_flag ) {
		...
		for ( col = print_col_start; col <= print_col_end; col++ ) {
		    printf ( "%f ", DEVA_image_data ( data_1d, row, col ) );
		}
		printf ( "\n" );
	    }
	    printf ( "\n" );
	...
	display = DEVA_RGB_image_new ( DEVA_image_n_rows ( data_1d ),
		DEVA_image_n_cols ( data_1d ) );

	/* find extremal value in input */
	max_1d = min_1d = 0.0;	/* quiet -Wunused-variable */
	found_valid_point = FALSE;
	for ( row = 0; row < DEVA_image_n_rows ( data_1d ); row++ ) {
	    for ( col = 0; col < DEVA_image_n_cols ( data_1d ); col++ ) {
		if ( DEVA_image_data ( data_1d, row, col ) < MAX_DST ) {
		    max_1d = min_1d = DEVA_image_data ( data_1d, row, col );
	...
	for ( row = 0; row < DEVA_image_n_rows ( data_1d ); row++ ) {
		...
		if ( max_1d < DEVA_image_data ( data_1d, row, col ) ) {
		    max_1d = DEVA_image_data ( data_1d, row, col );
		} else if ( min_1d > DEVA_image_data ( data_1d, row, col ) ) {
		    min_1d = DEVA_image_data ( data_1d, row, col );
		}
	    }
	}
	...
	for ( row = 0; row < DEVA_image_n_rows ( data_1d ); row++ ) {
		...
		if ( DEVA_image_data ( data_1d, row, col ) <= MAX_DST ) {
			...
			if ( fullrange && !reverse ) {
				...
				rescale ( DEVA_image_data ( data_1d, row,
					    col ), min_1d, max_1d,
					0.0, 1.0 );
				...
				rescale ( DEVA_image_data ( data_1d, row,
					    col ), min_1d, max_1d,
					0.0, 1.0 );
			} else if ( DEVA_image_data ( data_1d, row, col )
				< 0.0 ) {
			    display_value_float.red = norm_1d *
				-DEVA_image_data ( data_1d, row, col );
			    display_value_float.green = 0.0;
			    display_value_float.blue = 0.0;
			} else {
			    display_value_float.red = 0.0;
		...
		DEVA_image_data ( display, row, col ) =
	...
	DEVA_float_image_delete ( data_1d );
	...
	data_3d = DEVA_geom3d_from_radfilename ( argv[argpt++] );
	...
	if ( print_flag ) {
		...
		for ( col = print_col_start; col <= print_col_end; col++ ) {
		    printf ( "(%.2f,%.2f,%.2f) ",
			    DEVA_image_data ( data_3d, row, col ) . X,
			    DEVA_image_data ( data_3d, row, col ) . Y,
			    DEVA_image_data ( data_3d, row, col ) . Z );
		}
		printf ( "\n" );
	    }
	    printf ( "\n" );
	...
	display = DEVA_RGB_image_new ( DEVA_image_n_rows ( data_3d ),
		DEVA_image_n_cols ( data_3d ) );

	/* find extremal value in input */

	max_X = min_X = DEVA_image_data ( data_3d, 0, 0 ) . X;
	max_Y = min_Y = DEVA_image_data ( data_3d, 0, 0 ) . Y;
	max_Z = min_Z = DEVA_image_data ( data_3d, 0, 0 ) . Z;

	for ( row = 0; row < DEVA_image_n_rows ( data_3d ); row++ ) {
	    for ( col = 0; col < DEVA_image_n_cols ( data_3d ); col++ ) {

		if ( max_X <
			DEVA_image_data ( data_3d, row, col ) . X ) {
		    max_X = DEVA_image_data ( data_3d, row, col ) . X;
		} else if ( min_X >
			DEVA_image_data ( data_3d, row, col ) . X ) {
		    min_X = DEVA_image_data ( data_3d, row, col ) . X;
		}

		if ( max_Y <
			DEVA_image_data ( data_3d, row, col ) . Y ) {
		    max_Y = DEVA_image_data ( data_3d, row, col ) . Y;
		} else if ( min_Y >
			DEVA_image_data ( data_3d, row, col ) . Y ) {
		    min_Y = DEVA_image_data ( data_3d, row, col ) . Y;
		}

		if ( max_Z <
			DEVA_image_data ( data_3d, row, col ) . Z ) {
		    max_Z = DEVA_image_data ( data_3d, row, col ) . Z;
		} else if ( min_Z >
			DEVA_image_data ( data_3d, row, col ) . Z ) {
		    min_Z = DEVA_image_data ( data_3d, row, col ) . Z;
		}
	    }
	}
	...
	data_1d = DEVA_float_image_new ( DEVA_image_n_rows ( data_3d ),
		DEVA_image_n_cols ( data_3d ) );
	...
	switch ( dim_to_show ) {
	    case 'x':
		for ( row = 0; row < DEVA_image_n_rows ( data_3d ); row++ ) {
		    for ( col = 0; col < DEVA_image_n_cols ( data_3d );
			    col++ ) {
			DEVA_image_data ( data_1d, row, col ) =
			    DEVA_image_data ( data_3d, row, col ) . X;
		    }
	...
	max_1d = min_1d = DEVA_image_data ( data_1d, 0, 0 );
	...
	for ( row = 0; row < DEVA_image_n_rows ( data_1d ); row++ ) {
	    for ( col = 0; col < DEVA_image_n_cols ( data_1d ); col++ ) {
		if ( DEVA_image_data ( data_1d, row, col ) < 0.0 ) {
		    display_value_float.red = norm_1d *
			-DEVA_image_data ( data_1d, row, col );
		    display_value_float.green = 0.0;
		    display_value_float.blue = 0.0;
    ...
    if ( outline_flag ) {
	for ( row = print_row_start; row <= print_row_end; row ++ ) {
	    DEVA_image_data ( display, row, print_col_start ) =
		DEVA_image_data ( display, row, print_col_end ) = cyan;
	}

	for ( col = print_col_start; col <= print_col_end; col ++ ) {
	    DEVA_image_data ( display, print_row_start, col ) =
		DEVA_image_data ( display, print_row_end, col ) = cyan;
	}
    }

    DEVA_RGB_image_to_filename_png ( argv[argpt++], display );

    DEVA_RGB_image_delete ( display );
