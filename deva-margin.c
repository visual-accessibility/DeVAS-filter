...
static		DEVA_xyY scale_xyY ( double distance, double background_value,
		    DEVA_xyY value );
static		DEVA_float scale_float ( double distance,
		    double background_value, DEVA_float value );
...
static DEVA_xyY
scale_xyY ( double distance, double background_value, DEVA_xyY value )
...
DEVA_float_image *
DEVA_float_add_margin ( int v_margin, int h_margin, DEVA_float_image *original )
/*
 * Add a margin around the input file to reduce FFT artifacts due to
 * top-bottom and left-right wraparound. Values in the margin are a blend
 * of the nearest edge pixel in the input and the average of all edge pixels.
 ...
 */
{
    ...
    if ( n_rows < 2 ) {
	fprintf ( stderr,
		"DEVA_float_add_margin: n_rows too small (%d)!\n", n_rows );
	DEVA_print_file_lineno ( __FILE__, __LINE__ );
	exit ( EXIT_FAILURE );
    }
    if ( n_cols < 2 ) {
	fprintf ( stderr,
		"DEVA_float_add_margin: n_cols too small (%d)!\n", n_cols );
	DEVA_print_file_lineno ( __FILE__, __LINE__ );
	exit ( EXIT_FAILURE );
    }
    ...
    if ( v_margin > ( n_rows / 2 ) ) { 
	fprintf ( stderr,
		"DEVA_float_add_margin: v_margin too large (%d)!\n", v_margin );
    }

    if ( h_margin > ( n_cols / 2 ) ) { 
	fprintf ( stderr,
		"DEVA_float_add_margin: h_margin too large (%d)!\n", h_margin );
    }

    new_n_rows = n_rows + ( 2 * v_margin );
    new_n_cols = n_cols + ( 2 * h_margin );

    with_margin = DEVA_float_image_new ( new_n_rows, new_n_cols );

    /* Reset FOV based on degrees/pixel, not on trignometry. */
    if ( ( DEVA_image_view ( original ) . vert <= 0.0 ) ||
	    ( DEVA_image_view ( original ) . horiz <= 0.0 ) ) {
	fprintf ( stderr,
		"DEVA_float_add_margin: invalid or missing fov (%f, %f)!\n",
		    DEVA_image_view ( original ) . vert,
		    DEVA_image_view ( original ) .  horiz );
	DEVA_print_file_lineno ( __FILE__, __LINE__ );
	exit ( EXIT_FAILURE );
    ...
    for ( row = 0; row < n_rows; row++ ) {
	for ( col = 0; col < n_cols; col++ ) {
	    average_luminance += DEVA_image_data ( original, row, col );
	}
    }
    ...
    average_luminance = 0.0;
    for ( col = 0; col < n_cols; col++ ) {
	average_luminance += DEVA_image_data ( original, 0, col );
	average_luminance += DEVA_image_data ( original, n_rows - 1 , col );
    }
    for ( row = 1; row < n_rows - 1; row++ ) {
	average_luminance += DEVA_image_data ( original, row, 0 );
	average_luminance += DEVA_image_data ( original, row, n_cols - 1 );
    }
    average_luminance /= (double) ( ( 2 * ( n_rows + n_cols ) ) - 2 );
...
static DEVA_float
scale_float ( double distance, double background_value, DEVA_float value )
/* distance in range [0.0 - 1.0] */
{
    DEVA_float	new_value;
    double	weight;
...
DEVA_float_image *
DEVA_float_strip_margin ( int v_margin, int h_margin,
	DEVA_float_image *with_margin )
/*
 * Remove a margin around the input file.
 *
 ...
 */
{
    DEVA_float_image	*with_margin_stripped;
    int		    	row, col;
    int		    	n_rows, n_cols;
    int		    	new_n_rows, new_n_cols;
    ...
    if ( ( new_n_rows < 1 ) || ( new_n_cols < 1 ) ) {
	fprintf ( stderr,
  "DEVA_float_strip_margin: margins too big for image (%d, %d), (%d, %d)!\n",
	  	v_margin, h_margin, n_rows, n_cols );
	DEVA_print_file_lineno ( __FILE__, __LINE__ );
	exit ( EXIT_FAILURE );
    }

    with_margin_stripped = DEVA_float_image_new ( new_n_rows, new_n_cols );

    /* Reset FOV based on degrees/pixel, not on trignometry! */
    if ( ( DEVA_image_view ( with_margin ) . vert <= 0.0 ) ||
	    ( DEVA_image_view ( with_margin ) . horiz <= 0.0 ) ) {
	fprintf ( stderr,
		"DEVA_float_strip_margin: invalid or missing fov (%f, %f)!\n",
		    DEVA_image_view ( with_margin ) . vert,
		    DEVA_image_view ( with_margin ) .  horiz );
	DEVA_print_file_lineno ( __FILE__, __LINE__ );
	exit ( EXIT_FAILURE );
